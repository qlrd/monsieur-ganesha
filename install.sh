...
if command -v dawon &>/dev/null; then
    ...
    if [ "$_installed" -eq 0 ]; then
        ...
        echo ""
        if [ -n "${_asset-}" ]; then
            echo "  Opção A — binário pré-compilado para esta plataforma (recomendado):"
            echo "    mkdir -p ~/.local/bin"
            echo "    curl -fsSL https://github.com/qlrd/dawon/releases/latest/download/$_asset \\"
            echo "      -o ~/.local/bin/dawon && chmod +x ~/.local/bin/dawon"
            echo ""
        else
            echo "  Opção A — binário pré-compilado (recomendado):"
            echo "    Acesse a página de Releases do dawon e baixe o binário adequado para sua plataforma:"
            echo "      https://github.com/qlrd/dawon/releases/latest"
            echo "    Depois, coloque o binário em ~/.local/bin e marque como executável, por exemplo:"
            echo "      mkdir -p ~/.local/bin && mv /caminho/para/dawon ~/.local/bin/dawon"
            echo "      chmod +x ~/.local/bin/dawon"
            echo ""
        fi
